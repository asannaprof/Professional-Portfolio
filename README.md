# Professional Portfolio

A repository showcasing several artifacts from my professional portfolio.

## Overview

This repository contains a curated collection of work samples demonstrating my professional competencies across multiple domains, including technical documentation, project management, and professional development facilitation.

---

## Folder Contents

### Technical Documentation - Student Guides and Materials

The items in this folder were created to help students understand and complete academic tasks independently. These materials translate procedures, standards, and complex concepts into clear instructional guides that facilitate self-directed learning and skill development.

**Key Skills Demonstrated:**
- Clear technical writing
- Complex concept explanation
- Instructional design
- Student-centered documentation

---

### Professional Development - Instructional Materials for Adults

The items in this folder were created as supplements to professional development sessions I led for colleagues. They demonstrate my competencies with Microsoft Office tools like Word and PowerPoint, along with adult learning principles and instructional facilitation.

**Key Skills Demonstrated:**
- Microsoft Office proficiency (Word, PowerPoint)
- Adult learning principles
- Professional facilitation
- Training material development

---

### Project Planning - Master's Capstone Project

The materials in this folder document the development of my master's capstone project at McDaniel College. The project required me to transform a complex, open-ended assignment into a structured research effort, demonstrating advanced planning, organization, and project management skills.

**Key Skills Demonstrated:**
- Project planning and management
- Research methodology
- Complex problem-solving
- Academic project organization

---

## About This Portfolio

This repository reflects my commitment to excellence in:
- **Technical Communication** - Creating clear, accessible documentation
- **Instructional Design** - Developing effective learning materials
- **Professional Development** - Facilitating adult learning
- **Project Management** - Planning and executing complex initiatives

Feel free to explore the folders above to review specific artifacts and work samples.
